...
\begin{minipage}{0.45\textwidth}
  \centering
  \subimport{../tables/}{lengths-V2-state-vector-PRELIM}
\end{minipage}
\hfill
\end{table}
...
Interestingly, the verification run-time for \LTLPredicate{HLT} and \LTLPredicate{FSU} does not appear strongly correlated with the \(T\) and \(N\) parameters.
...
\begin{figure}[ht!]
\centering
\caption[Runtime projections for \CGKAmod{\VersionOne}{T}{N}]{%
\label{fig:runtime-projections}%
Run-time (wall clock) projections for \CGKAmod{\VersionOne}{T}{N} using $T \in [4, 7]$ run-time observations as predictors for $T \in [8, 12]$ run-time.
}%
\includegraphics[width=0.8\textwidth]{./figures/fsu-pcs-runtime-combined-preidcted.png}
\end{figure}

\begin{table}[ht!]                                                                                                                      
\caption[Run-time predictions for \CGKAmod{\VersionOne}{T}{N}]{%
\label{tab:predicted-run-times}%
Run-time (wall clock) predictions for \CGKAmod{\VersionOne}{T}{N} using $T \in [4, 7]$ as predictors for $T \in [8, 12]$.
}%
\subimport{../tables/}{runtime-prediction-V1}
\end{table}
...
As indicated in Table\ \ref{tab:verification-hypotheses}, the first protocol \VersionOne\ is known to have a \Abrev{FSU} security deficiency, so the hypothesis is that verification of \LTLPredicate{FSU} should fail for \emph{at least} \CGKAmod{\VersionOne}{12}{8}.
