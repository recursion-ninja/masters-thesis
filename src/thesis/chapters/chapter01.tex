...
The \Abrev{AS} is a trusted third party which is expected to carry out it's required tasks without failure.
...
Both \Abrev{FS} and \Abrev{PCS} have been researched with respect to two-party \Abrev{SM}, producing provably secure as well as efficient constructions.
...
If the state of any group member is leaked at some point, all previous update secrets remain hidden from the adversary.
\end{definition}
