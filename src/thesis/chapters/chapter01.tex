...
Finally, the nondeterministic computation allows, without loss of generality, group members to be removed from the model as process agents, dramatically reducing the model's state-space (see Section \ref{sec:elided-group-members}).
...
A contrast of \Abrev{SM} and \Abrev{SGM} is illustrated in Figure \ref{fig:Secure-Messaging-And-Secure-Group-Messaging}.
...
Similarly, \Abrev{SGM} differs in interesting ways from broadcast encryption \autocite{fiat1993broadcast}.
Both \Abrev{SGM} and broadcast encryption are multi-party communication protocols, but the communication conducted via the protocols are quite different.
...
However, in the case of \Abrev{SGM} \emph{all} group members can send and receive communications as well as add or remove group members.
Broadcast encryption can be thought of as a form of \Abrev{SGM} where only one group member broadcasts messages, adds group members, removes group members, and instructs group members to update their encryption keys; with all other group members only receiving messages.
Given that \Abrev{SGM} provides equal privilege to each group member, security proofs are more complicated when compared to broadcast encryption. 
...
Originally produced in 2018, \Abrev{MLS} seeks to specify a standardization within which \Abrev{SGM} protocols can be defined.
...
The \Abrev{MLS} development has recently experienced conceptual adoption, with core notions being referenced and expanded upon in new works \autocite{Boyd2020}.
...
Importantly, the \Abrev{DS} will always deliver messages from all senders to a other group members in a consistent order.
Consistent ordering of delivery of broadcasted messages allows the \Abrev{DS} some flexibility in resolving concurrency issues which emerge during asynchronous communication.
...
An example of this acknowledgment behavior is depicted in Figures \ref{fig:Secure-Messaging-Broadcast} and \ref{fig:Secure-Messaging-Update}.
...
\begin{figure}[ht!]
  \centering
  \caption[Secure Messaging Broadcast with Acknowledgment]{%
  \label{fig:Secure-Messaging-Broadcast}%
  Secure Messaging Broadcast with Acknowledgment:
  \begin{enumerate}
...
\begin{figure}[ht!]
  ...
  \begin{enumerate}
    ...
    \item Alice polls the \Abrev{DS} and receives update instructions
  \end{enumerate}
  }%
...
The security guarantees of \Abrev{MLS} includes many ``solved problems'' in the field of cryptography with respect to the \Abrev{ITM}; notably: end-to-end encryption, message confidentiality, message integrity, message authentication, and membership authentication \autocite{rfc4949}.
...
While efficient scaling up to a group containing 50,000 member is the target of all \Abrev{MLS} protocols, no guidance is given for larger groups.
...
An illustration of epochs for a \Abrev{SM} protocol is included in Figure \ref{fig:Secure-Messaging-Timeline}, though the same delineation applies to \Abrev{SGM} protocols as well.
...
\Abrev{FS} is a desirable security guarantee which offers protection in the case that a protocol's long-term secret key(s) are compromised \autocite{boyd2021modern}.
...
For \Abrev{SGM}, this same notion has been extended to \Abrev{CGKA} \autocite{alwen2020security} which is used as an abstraction for reasoning about the \Abrev{MLS} security guarantees.
A continuous group key-agreement scheme is defined over a set of local member states $\gamma \in \Gamma$, a set of welcome instruction messages for adding a member to a group $w \in W$, a set of update instruction messages $u \in U$, and a set of cryptographic secrets $i \in I$.
...
In essence, the \Abrev{DS} mediates asynchronous epoch continuity among group members.
...
An oracle is an agent in the game which the adversary can query to receive information or have an action performed on their behalf.
...
The first six oracles call the \CGKAdef\ algorithms internally and can be intuitively thought of as thin wrappers exposed to \(\mathcal{A}\) for the corresponding algorithms from the \CGKAdef\ definition.
...
The \Abrev{MLS} specification is a standardization, which protocols may conform to by satisfying the \Abrev{SGM} use case, maintaining continuous group key agreement as defined by \Abrev{CGKA}, as well as providing efficiency and scalability requirements.
...
Using the notation introduced above, the \Abrev{CGKA} abstraction for TreeKEM produces the \CGKAmod{\VersionOne}{}{} model which will be verified.
Verifying the correctness of the \Abrev{CGKA} algorithm implementations TreeKEM is beyond the scope of this work.
