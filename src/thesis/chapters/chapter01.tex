...
This generalization to multi-party communication is known as \Abrev{SGM}.
...
\Abrev{SGM} described the \emph{use case} of secure multi-party communication.
...
The \Abrev{SGM} requirement of adding and removing participants to the secure communication channel poses interesting open challenges in the field of cryptography.
...
The Internet threat model of RFC3552 \autocite{rescorla2003rfc3552} is the context within which the \Abrev{MLS} specifies it's security guarantees.
...
\Abrev{MLS} specifies the existence of an authentication service (AS) from which a messenger can request fresh public key for a specified contact which can be immediately used within the \Abrev{MLS} protocol.
An advantage of using the AS as described in \Abrev{MLS} is that it allows the contact to asynchronously query the AS for the corresponding secret key at a later time, ameliorating potential protocol synchronization issues.
In addition to the AS, \Abrev{MLS} specifies the existence of a delivery service (DS).
...
The security guarantees of \Abrev{MLS} includes many ``solved problems'' in the field of cryptography with respect to the Internet threat model; notably: end-to-end encryption \autocite{padlipsky1978limitations}, message confidentiality (CITE), message integrity \autocite{voydock1983security}, message authentication \autocite{jueneman1983message}, and membership authentication \autocite{chaum1985showing}.
However, \Abrev{MLS} also specifies interesting open problems of (perfect) forward secrecy (PFS) \autocite{gunther1989identity} and post-compromise security (PCS) \autocite{cohn2016post} as security guarantees of the key-agreement protocol.

Both PFS and PCS have been researched with respect to bidirectional \Abrev{SM}, producing provably secure as well as efficient constructions.
...
However, the requirement of PFS and PCS laid out by the \Abrev{MLS} specification for a \Abrev{SGM} protocol has introduced a novel area of research, as the double-ratchet-based algorithms do not directly map to multi-party communication.
...
As the \Abrev{MLS} draft specification has gone through over a dozen revisions in the last half decade, the concurrent development and refinement of the TreeKEM protocol has significantly informed the direction and language of \Abrev{MLS}.
...
Forward Secrecy and Post Compromise Secrecy for TreeKEM, as well as other \Abrev{SGM} protocols, are delineated in said epochs.
...
PFS has been used in bidirectional key agreement communication protocols, including Transport Layer Security (TLS) (CITE) protocols such as OpenSSL (CITE), as well as \Abrev{SM} protocols such as the Signal Protocol (CITE).
While the definition of FS has been explored and made rigorously clear for bidirectional key agreement protocols, the notion of FS requires extension in the multi-directional \Abrev{MLS} specification.
The following is the definition of FS in the multi-directional protocol context \autocite{alwen2020security} which has been adopted by \Abrev{MLS}.
...
However, within the multi-directional \Abrev{MLS} specification, the difference between FS and PCS has been separated to describe different security guarantee protocol when one or more of the protocol's long-term secret key(s) are compromised.
...
The following definition of PCS in the multi-directional protocol context \autocite{alwen2020security} which has been adopted by \Abrev{MLS}.
...
The \Abrev{CGKA} security game defines the following oracles:
...
The DS is guaranteed by the \Abrev{MLS} specification to provide all parties with a consistent presentation of messaging in and between epochs.
...
An important consequence of maintaining this consistent view within the context of the adversarial \Abrev{CGKA} game is that, when the attacker queries the \textbf{deliver} oracle enabling a chosen epoch transition from \(t\) to \(t+1\), the DS must discard all other epoch transition messages with originated in a previous epoch.
...
The \textbf{chall} oracle allows the attacker to signal that they believe that they have succeeded in violating the \Abrev{CGKA} protocol's security and the game should immediately cease so the attacker can provide proof of their successful attack by demonstrating \emph{advantage}.
...
An attacker proving \emph{advantage} is defined by the \Abrev{CGKA} security game as a parameterized expression involving oracles of the game.
...
The \((T, C, N)\)-attacker \(\mathcal{A}\) proves \emph{advantage} by ending the game via a query to the \textbf{chall} oracle, winning the \Abrev{CGKA} security game, if and only if the attacker:
...
as well as in Section \ref{sec:game-oracles} modeling the translations of the ten \Abrev{CGKA} security game oracles explicitly for TreeKEM.
...
\begin{figure}
\centering
\includegraphics[width=0.5\textwidth,height=5cm]{figures/venn-protocols.png}
\caption{Specificity relationship between definitions \label{fig:venn-protocols}}
\end{figure}

The definition of \Abrev{CGKA} and the construction of the \Abrev{CGKA} security game offers an abstraction between the protocol specific implementation and the desired security guarantees of FS and PCS specified by \Abrev{MLS}.
An equivalence proof of non-adaptive \Abrev{CGKA} security \ref{def:CGKA-Security} with FS and also with PCS accompanies its original description \autocite{alwen2020security}.
This work relies on the equivalence proof in order to soundly use \Abrev{CGKA} as a verifiable abstraction.
The equivalence proof results in the correlary that an attacker playing the \Abrev{CGKA} security game can only gain advantage by exploiting side-channel attack on the given \Abrev{CGKA}-conforming protocol.
A model of the \Abrev{CGKA} security game for a given \Abrev{CGKA}-conforming protocol, which verifies FS and PCS at the abstraction level, will consequently verify the FS and PCS security guarantees for the protocol, assuming that the protocol implementation is defect-free.
...
However, the \Abrev{CGKA} abstraction cannot verify that a given implementation of a protocol is defect-free implementation.
A separate and specific verification effort is required to prove that any chosen implementation provides the a correct version of the \Abrev{CGKA} algorithms.
All results presented by this work assume that the TreeKEM algorithm's implementation is correct.
The verification shows that, if given an implementation of TreeKEM which provides defect-free algorithms required by the \Abrev{CGKA} definition, then the protocol also provides the FS and PCS security guarantees due to an absence of side channel attacks.
Furthermore, the verification results reduce future obligations to derive verification of FS and PCS for all \Abrev{CGKA}-conforming protocol implementations to only verify algorithm correctness, and provide an a protocol modeling of the potential side channel information.
