...
One semantic which is invariant of a specific protocol, emerges from querying the \Oracle{add-user}{}, \Oracle{remove-user}{}, and \Oracle{send-update}{} oracles, conferring onto the attacker the ability to guide the protocol execution by instructing group members to begin new epochs on whichever terms the attacker considers favorable.
...
The equivalence proof results in the corollary that an attacker playing the \CGKAsec can only gain advantage by exploiting side-channel attack on the given \Abrev{CGKA}-conforming protocol.
