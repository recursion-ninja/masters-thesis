...
\begin{itemize}
  \item For some \(t\),\\~\hspace{4em}\(\mathcal{A}\) querries \Oracle{reveal}{t}    two or more times throught the \CGKAsec.  
  \item For some \(i\),\\~\hspace{4em}\(\mathcal{A}\) querries \Oracle{no-del}{ID_i} two or more times throught the \CGKAsec.
  \item For some \(i\), and some \(t\),\\~\hspace{4em}\(\mathcal{A}\) querries \Oracle{corr}{ID_i} two or more times during epoch \(t\).
\end{itemize}
...
Note that in each possible epoch transtion, \(\mathcal{A}\) decides which control message is delivered defining the next epoch.
