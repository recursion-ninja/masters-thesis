...
There are generally three classses of verification techinques which can be applied; logical systems, theorem proving, and model checking \autocite{kemmerer1994three}.
...
Logical systems for defining and reasoning about protocols, such as Linear Logic Framework \autocite{CERVESATO200219}, can produce increadibly strong verification results.
...
In the past, \Abrev{BAN logic} \autocite{burrows1989logic} was the tool of choice for verifying cryptographic protocols due to acheiving such a desireable balance.
...
Some domain specific theroem provers, such as Tamarin \autocite{basin2017symbolically}, exist for cryptography and communication protocols to reduce the required human effort.
...
Symbolic model checking via prcocess algebras, such as the communication between sequential proccess (CSP) algebra \autocite{Hoare1978}, has been use to great effect \autocite{roscoe1994model, Ryan1999, ryan2001modelling} and influenced the development of subsequent model checking approaches.
Explicit state model checking, contrastingly, enumerates the entire protocol state-space rather than cleverly applying abstract or algebreic transformations to derive verification proofs.
...
Among the many options of model checkers, I have chosen use the SPIN model checker \autocite{HolzmannSpinBook2003}.
