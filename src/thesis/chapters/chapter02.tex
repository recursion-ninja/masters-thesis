...
Unfortunately, protocols verified by \Abrev{BAN logic} were later found to have undiscovered attacks \autocite{10.5555/188307.188350}.
...
\Abrev{BAN logic} has since fallen out of favor and instead been replaced by model checking techniques \autocite{marrero1997model} and theorem proving \autocite{hoare1969axiomatic}, which have been able to produce attacks on cryptographic protocols which BAN techniques incorrectly verified as secure.

Theorem proving, when possible, is the preferred method of cryptographic protocol verification \autocite{bolignano1997towards}.
...
The model checker then asserts that one or more properties holds for the protocol by traversing through all combinations of transitions through the graph \autocite{ORBi-a8d60ab6-1101-4434-9511-c01ea4e5a15b}.
...
Once we decided to use model checking with \Abrev{LTL} as the formal verification methodology, we must then select the appropriate tools to use - the Protocol Meta Language (PROMELA) \autocite{holzmann1980basic, holzmann1990design}.
...
Once PROMELA is used to produce all possible states and transitions of TreeKEM, we must supply that representation to a model checker which will verify \Abrev{LTL} specifications.
Among the many options of model checkers, we will use the SPIN model checker \autocite{godefroid1990using, holzmann1997model}.
