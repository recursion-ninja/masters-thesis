...
Here the verification results for the models \CGKAmod{\VersionOne}{}{} and \CGKAmod{\VersionTwo}{}{} are presented.
...
At the time of submitting this thesis draft, not all verification prcoesses have completed, and are indicated by a \NA in table entries.

\hypertarget{performance-tuning}{%
\section{Performance Tuning}\label{performance-tuning}}

Prior to the verification results of LTL predicates on \CGKAmod{}{}{}, the performance and scaling of the verification efforts are contextualized.
...
\begin{table}[h!]
  ...
  \caption{State vector byte length for each model parameterization of \CGKAmod{\VersionOne}{}{}.\\Note that the model's state vector is indepenent of \Abrev{LTL} predicate $\varphi$}
  \subimport{../tables/}{state-vector-byte-length-PRELIM}
\end{table}


\hypertarget{verification-outcomes}{%
\section{Verification Outcomes}\label{verification-outcomes}}
...
While \LTLPredicate{HLT} is not truly a security property of \CGKAmod{}{}{}, it is used as a sanity check to ensure that the model verification is being performed correctlt on more complex \Abrev{LTL} predicates.
Verification results for \LTLPredicate{HLT} is displayed in Table \ref{tab:V1-HLT}.

\begin{table}[h!]
  \centering
  \label{tab:V1-HLT}
  \caption{Performance results of $\CGKAmod{\VersionOne}{T}{N} \models \LTLPredicate{HLT}$}
...
\begin{table}[h!]
  \centering
  \label{tab:V2-HLT}
  \caption{Performance results of $\CGKAmod{\VersionTwo}{T}{N} \models \LTLPredicate{HLT}$}
  \subimport{../tables/}{results-V2-HLT-PRELIM}
\end{table}
...
\begin{table}[h!]
  \centering
  \label{tab:V1-PCS}
  \caption{Performance results of $\CGKAmod{\VersionOne}{T}{N} \models \LTLPredicate{PCS}$}
...
\begin{table}[h!]
  \centering
  \label{tab:V2-PCS}
  \caption{Performance results of $\CGKAmod{\VersionTwo}{T}{N} \models \LTLPredicate{PCS}$}
  \subimport{../tables/}{results-V2-PCS-PRELIM}
\end{table}
...
\begin{table}[h!]
  \centering
  \label{tab:V1-FSU}
  \caption{Performance results of $\CGKAmod{\VersionOne}{T}{N} \models \LTLPredicate{FSU}$}
...
\begin{table}[h!]
  \centering
  \label{tab:V2-FSU}
  \caption{Performance results of $\CGKAmod{\VersionTwo}{T}{N} \models \LTLPredicate{FSU}$}
  \subimport{../tables/}{results-V2-FSU-PRELIM}
\end{table}
