...
\]
\begin{quotation}
``It is always the case that, when a new epoch begins and there are no users \(\mathtt{ID_{i}}\) corrupted via querying \Oracle{corr}{ID_{i}} which have not also suqsequently been involoved in a query to \Oracle{send-update}{ID_{i}} or \Oracle{remove-user}{ID_{*}}{ID_{i}}, then \(\mathcal{A}\) has not learned the shared secret key for the current epoch.''
\end{quotation}\\
\end{LTL}
